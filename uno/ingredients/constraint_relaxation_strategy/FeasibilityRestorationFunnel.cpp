...
void FeasibilityRestorationFunnel::initialize(Iterate& initial_iterate) {
...
Direction FeasibilityRestorationFunnel::compute_feasible_direction(Statistics& statistics, Iterate& current_iterate,
   ...
   if (this->current_phase == Phase::OPTIMALITY) {
      try {
         ...
         if (direction.status == SubproblemStatus::INFEASIBLE || !this->optimality_phase_strategy->current_iterate_acceptable_to_funnel) {
...
bool FeasibilityRestorationFunnel::is_iterate_acceptable(Statistics& statistics, Iterate& current_iterate, Iterate& trial_iterate, const Direction& direction,
      double step_length) {
...
GlobalizationStrategy& FeasibilityRestorationFunnel::current_globalization_strategy() const {
   return (this->current_phase == Phase::OPTIMALITY) ? *this->optimality_phase_strategy : *this->restoration_phase_strategy;
}

void FeasibilityRestorationFunnel::set_trust_region_radius(double trust_region_radius) {
