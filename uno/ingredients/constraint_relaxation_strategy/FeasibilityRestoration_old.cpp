...
#include <cassert>
#include <functional>
#include "FeasibilityRestoration_old.hpp"
#include "ingredients/globalization_strategy/GlobalizationStrategyFactory.hpp"
#include "ingredients/subproblem/SubproblemFactory.hpp"

FeasibilityRestorationOld::FeasibilityRestorationOld(Statistics& statistics, const Model& model, const Options& options) :
...
void FeasibilityRestorationOld::initialize(Iterate& initial_iterate) {
...
Direction FeasibilityRestorationOld::compute_feasible_direction(Statistics& statistics, Iterate& current_iterate, bool evaluate_functions) {
...
Direction FeasibilityRestorationOld::solve_optimality_problem(Statistics& statistics, Iterate& current_iterate, bool evaluate_functions) {
...
Direction FeasibilityRestorationOld::solve_feasibility_problem(Statistics& statistics, Iterate& current_iterate, bool evaluate_functions) {
   if (this->current_phase == Phase::OPTIMALITY) {
...
Direction FeasibilityRestorationOld::solve_feasibility_problem(Statistics& statistics, Iterate& current_iterate, const std::vector<double>& initial_point,
      bool evaluate_functions) {
   this->subproblem->set_initial_point(initial_point);
   return this->solve_feasibility_problem(statistics, current_iterate, evaluate_functions);
}

void FeasibilityRestorationOld::compute_progress_measures(Iterate& current_iterate, Iterate& trial_iterate, const Direction& direction,
      double step_length) {
...
void FeasibilityRestorationOld::switch_to_feasibility_restoration(Iterate& current_iterate) {
...
void FeasibilityRestorationOld::switch_to_optimality(Iterate& current_iterate, Iterate& trial_iterate) {
...
bool FeasibilityRestorationOld::is_iterate_acceptable(Statistics& statistics, Iterate& current_iterate, Iterate& trial_iterate, const Direction& direction,
      double step_length) {
...
const NonlinearProblem& FeasibilityRestorationOld::current_reformulated_problem() const {
   if (this->current_phase == Phase::OPTIMALITY) {
...
GlobalizationStrategy& FeasibilityRestorationOld::current_globalization_strategy() const {
   return (this->current_phase == Phase::OPTIMALITY) ? *this->optimality_phase_strategy : *this->restoration_phase_strategy;
}

void FeasibilityRestorationOld::set_trust_region_radius(double trust_region_radius) {
   this->subproblem->set_trust_region_radius(trust_region_radius);
}

void FeasibilityRestorationOld::set_infeasibility_measure(Iterate& iterate) {
   if (this->current_phase == Phase::OPTIMALITY) {
...
double FeasibilityRestorationOld::generate_predicted_infeasibility_reduction_model(const Iterate& current_iterate, const Direction& direction,
...
void FeasibilityRestorationOld::set_optimality_measure(Iterate& iterate) {
   if (this->current_phase == Phase::OPTIMALITY) {
      // scaled objective
...
std::function<double (double)> FeasibilityRestorationOld::generate_predicted_optimality_reduction_model(const Iterate& current_iterate,
...
size_t FeasibilityRestorationOld::get_hessian_evaluation_count() const {
   return this->subproblem->get_hessian_evaluation_count();
}

size_t FeasibilityRestorationOld::get_number_subproblems_solved() const {
